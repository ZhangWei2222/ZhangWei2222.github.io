...
![https://img.youpin.mi-img.com/luban/v6aa5k3mcoo_22026080841621842545997.png](/Users/wei/Desktop/v6aa5k3mcoo_22026080841621842545997.png)

## 实际项目中的坑

实际项目中，中间的画布首先是通过 dom-to-image 工具将 dom 转成 image，再绘成 canvas。同时中间的 dom 可以进行缩放（因此，生成的图片尺寸也需要根据缩放比例进行缩放）。如果按上面所述的常规方法，dom 转 canvas 时会出现画布模糊的效果（在小屏里面）。需要注意等比缩放

1. 首先需要获取设备像素比 devicePixelRatio：物理像素/css 像素
2. 等比缩放：用得到的 devicePixelRatio，加到所有有关计算的地方，比如绘制 canvas 的宽高，放大镜的宽高等等
...
// 将 canvas 放大到设备像素比来绘制，最后将 canvas 压缩成一倍的物理大小来展示
myCanvas.style.width = myCanvas.width + "px";
myCanvas.style.height = myCanvas.height + "px";
myCanvas.width = myCanvas.width * getPixelRatio;
myCanvas.height = myCanvas.height * getPixelRatio;
```

## [仓库地址](https://github.com/ZhangWei2222/color-straw)
